...
public class FxMacro implements IMacro {
	
	private static final Pattern PATTERN = Pattern.compile("(.*)<p>\\.fx:\\s?(.*?)</p>(.*)", Pattern.DOTALL);
	...
	public ContentAndClasses process(String content, File parentDir) {
		...
		if(matcher.matches()) {
			...
			String[] classNames = matcher.group(2).split(" ");
			for(String className : classNames) {
				cc.addClass(className);
			}
		}
